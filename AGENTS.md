...
| `src/config/` | Game configuration — Phaser config (`game.ts`), defender type registry (`defenders.ts`), enemy type registry (`enemies.ts`), level configs LEVEL_1-LEVEL_10 and ALL_LEVELS (`levels.ts`) |
...
| `src/entities/` | Phaser game objects — `DefenderEntity.ts` (per-key shape drawing incl. trapper/mine, idle animations, combat reactions), `EnemyEntity.ts` (per-key shape drawing incl. armored/jumper/boss, movement animations, hit flash, helmet degradation, per-type scale), `ProjectileEntity.ts` (yellow circle for shooter, cyan for cannon via `isCannon` flag, amber for honey bear via `isHoney` flag) |
...
- **Defenders**: 7 types — Jack-in-the-Box/generator (drops collectible sparks at its position, no passive income), Water Pistol/shooter (ranged projectile attack, damage 15), Block Tower/wall (cheap sacrificial blocker, cost 25, health 90), Water Cannon/cannon (powerful ranged shooter, damage 24, cost 150, range 9, knockback 1 cell on hit — pushes non-boss enemies rightward, boss immune; cyan projectile, larger radius), Glitter Bomb/bomb (single-use, instant 3×3 AOE detonation on placement — ±1 row ±1 col, instant-kills non-boss, BOMB_BOSS_DAMAGE=400 for boss, sparkle burst VFX, recharge 15s, cost 100), Honey Bear/trapper (projectile-based area-denial — fires slow amber projectile at nearest lane enemy, on hit applies AOE damage to target row ±1 row via `applyAOEDamage` and creates honey pools per affected row via `createHoneyPot`; pools slow enemies to 0.5x speed for 8s; constants: HONEY_BEAR_PROJECTILE_SPEED=2, HONEY_BEAR_FIRE_INTERVAL=3, HONEY_BEAR_AOE_DAMAGE=5), Marble Mine/mine (single-use, arms after 6s delay, instant-kills first enemy on cell overlap, does not block movement, recharge 20s). DefenderType interface has `behavior`, `singleUse`, `knockback`, and `bio` fields. Recharge timers control cooldown between single-use placements.
- **Enemies**: 5 types — Dust Bunny/basic (0.30 cells/s, low health, scale 1.0), Cleaning Robot/tough (0.15 cells/s — slowest, high health, scale 1.35, gear bolt details), Armored Bunny/armored (0.20 cells/s — slower than basic, 3x basic health, scale 1.15, helmet with 3-stage visual degradation), Sock Puppet/jumper (0.36 cells/s — fastest, scale 0.85, spring coil detail, jumps over first defender encountered via `jumpsRemaining`), Mega Mop/boss (0.08 cells/s, 2000 HP, scale 1.6, bossType=true — unique mop-headed brute shape, stomp movement animation; mines deal MINE_BOSS_DAMAGE=400 instead of instant kill). Speed hierarchy: jumper > basic > armored > tough > boss. EnemyType interface has optional `jumpsRemaining`, `armorStages`, `scale`, `bio`, and `bossType` fields. Scale applies to Graphics shape rendering; health bar and flash overlay adjust proportionally.
...
- **Level progression**: 10 guided levels (ALL_LEVELS length 10) with PvZ-style pacing — gentle in stage one so most players clear L1 comfortably. Spawn intervals widen to 2.0s minimum across L1-L9; each level is a gradual step up (L1 min gap 5.0s → L9 min gap 2.0s). L10 breaks the 2.0s floor intentionally for climactic pacing. L1: 1 lane (center row), tutorial mode, 1 wave, starting balance 100. L2: 3 lanes, 2 waves, basic enemies. L3: 5 lanes, 3 waves, basic enemies. L4: 5 lanes, 3 waves, basic enemies (Block Tower available). L5: 5 lanes, 4 waves, basic + armored from wave 2 (pre-round enemy bio, 25s setup delay). L6: 5 lanes, 4 waves, basic + Cleaning Robot from wave 2 (pre-round enemy bio, 25s setup delay). L7: 5 lanes, 4 waves, Cleaning Robots in every wave (Honey Bear practice, first loadout selection — 6 unlocked > 5 max). L8: 5 lanes, 4 waves, no Sock Puppets in wave 1, jumpers from wave 2 (pre-round enemy bio, 25s setup delay). L9: 5 lanes, 5 waves, all 4 enemy types. L10: 5 lanes, 5 waves, formation rush (waves 1-4 at 0.5-0.8s intervals), wave 5 = single Mega Mop boss spawn (pre-round enemy bio, 20s setup delay) — climactic boss fight completing the first full playthrough arc. LevelConfig includes `startingBalance`, `tutorialMode`, `enemyBio`, and `setupDelay` fields. Level progress persisted to localStorage (`zombo_progress`). LevelSelectScene shows locked/unlocked/completed states.
- **Defender unlocks**: L1 start = [generator, shooter]. UNLOCK_MAP: completing L3 unlocks wall (Block Tower) + cannon (Water Cannon), completing L5 unlocks bomb (Glitter Bomb), completing L6 unlocks trapper (Honey Bear), completing L8 unlocks mine (Marble Mine). Unlock state persisted to localStorage (`zombo_unlocks`). Pre-level loadout selection appears when player has > 5 unlocked defenders (max 5 selectable, MAX_LOADOUT=5) — first triggered naturally before L7. Loadout cards use proportional sizing derived from GAME_WIDTH/GAME_HEIGHT (no hardcoded px), with staggered entry animation, selection bounce tween, idle bob on defender previews, and 2-row grid layout when 6+ defenders are shown (top row ceil(n/2), bottom row centered).
...
- **SingleUse**: pure logic for mine trigger (`mineTriggerCheck`, `MineState` arm timer) and bomb detonation (`bombDetonate`, `BOMB_BOSS_DAMAGE`). No Phaser dependency.
...
| Honey pools | 2 | Amber honey pot ground hazards (alpha 0.35, no pointer input) |
